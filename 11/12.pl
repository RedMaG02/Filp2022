...
polinomo(_,_,0,_,0):-!.
polinomo(A,B,K,N,B):- NEWN is N + 1, CHISLO is abs((N*N) + (A*N) + B), prost(CHISLO,BOOLK), (BOOLK =:= 0 -> NEWK is K+1,NEWB is 1; NEWK is K, NEWB is 0), polinomo(A,B,NEWK,NEWN,NEWB).
