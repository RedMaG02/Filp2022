...
prost(X,ANSWER,DEL):- NEWDEL is DEL -1, prost(X,ANSWER1,NEWDEL),DELEN is X mod DEL, (DELEN =:= 0 -> ANSWER is ANSWER1 + 1; ANSWER is ANSWER1).

polinom(A,B,K):-polinom(A,B,K,0,1).
polinom(_,_,K,_,0):-K is 1 - 1 ,!.
polinom(A,B,K,N,B):- NEWN is N + 1, CHISLO is abs((N*N) + (A*N) + B), prost(CHISLO,BOOLK), (BOOLK =:= 0 -> NEWK is K+1,NEWB is 1; NEWK is K, NEWB is 0), polinom(A,B,NEWK,NEWN,NEWB).

%method13(ANSWER,A,B,P,MAX)
method13(ANSWER):-method13(ANSWER,(-999),(-999),(-1000000),0).
method13(P,999,1000,P,_):-print(P),!.
method13(ANSWER,A,1000,P,MAX):- NEWA is A +1, method13(ANSWER,NEWA,(-999),P,MAX).
method13(ANSWER,A,B,P,MAX):- NEWB is B+1, polinom(A,B,K), (K>MAX -> NEWMAX is K, NEWP is A * B; NEWMAX is MAX, NEWP is P), method13(ANSWER,A,NEWB,NEWP,NEWMAX).
